...
tibble(a = 0,
  ...
  mutate(prob = dunif(parameter_space, a, b),
         b    = str_c("b = ", b)) %>% 
  gf_ribbon(0 + prob ~ parameter_space) %>%
  gf_facet_wrap( ~ b, ncol = 5) %>%
  gf_refine(
    scale_x_continuous(breaks = c(0, 1:3, 9)),
    scale_y_continuous(breaks = c(0, 1/9, 1/3, 1/2, 2/3, 1),
                         labels = c("0", "1/9", "1/3", "1/2", "2/3", "1"))
...
```{r, fig.width = 2.5, fig.height = 2, include = use_ggformula, eval = use_ggformula}
gf_dist("beta", shape1 = 1, shape2 = 1, geom = "area") %>%
...
```{r}
Grid1 <-
    expand_grid(p = seq(from = 0, to = 1, length.out = 20),      # define grid
           prior  = 1) %>%                                       # define prior
    mutate(likelihood = dbinom(6, size = 9, prob = p)) %>%       # compute likelihood at each value in grid
...
```{r}
Grid2 <-
    expand_grid(p = seq(from = 0, to = 1, length.out = 5),       # define grid
           prior  = 1) %>%                                       # define prior
    mutate(likelihood = dbinom(6, size = 9, prob = p)) %>%       # compute likelihood at each value in grid
    mutate(unstd_posterior = likelihood * prior) %>%             # compute product of likelihood and prior
    mutate(posterior = unstd_posterior / sum(unstd_posterior))   # standardize the posterior, so it sums to 1
