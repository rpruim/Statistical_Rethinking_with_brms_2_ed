...
This time, let's make a little function to produce our grid data frame given:
...
BinomialGrid6of9 %>% reactable::reactable()
```

This function returns a data frame containing all the components in McElreath's **R** code 3.2 block. 
...
Now we'll use the `dplyr::slice_sample()` function to sample rows from `d`, saving them as `sample`.
...
Now we can plots in Figure 3.1. For the left panel, we'll need to add a variable numbering the samples.

```{r, fig.width = 3.5, fig.height = 3, include = !use_ggformula, eval = !use_ggformula}
p1 <-
  Samples6of9 %>% 
  mutate(sample_number = 1:n()) %>% 
  
  ggplot(aes(x = sample_number, y = p)) +
...
```{r, fig.width = 7.0, fig.height = 3, include = use_ggformula, eval = use_ggformula}
library(patchwork)
p1 | p2
```
...
An even trickier approach for the same is to insert the logical statement `p < .5` within the `mean()` function.

```{r}
Samples6of9 %>%
  summarise(sum = mean(p < .5))
```
...
Here are some additional (and arguably more natural) ways to do this taking advantage of functions in the **mosaic**
package.

```{r}
prop( ~ p < 0.5, data = Samples6of9)
df_stats( ~ p < 0.5, data = Samples6of9, props)
tally( ~ compare (p, 0.5), data = Samples6of9, format = "prop")
```
...
Samples6of9 %>% 
  filter(p > .5 & p < .75) %>% 
  summarise(sum     =       n() / nrow(Samples6of9),
            percent = 100 * n() / nrow(Samples6of9))
```
...
```{r}
Samples6of9 %>%
  summarise(percent = 100 * mean(p > .5 & p < .75))
```
...
```{r, fig.width = 6, fig.height = 2.5, include = !use_ggformula, eval = !use_ggformula}
# upper left panel
p1 <-
  BinomialGrid6of9 %>% 
  ggplot(aes(x = p, y = posterior)) +
  geom_line() +
  geom_ribbon(data = BinomialGrid6of9 %>% filter(p < .5),
...
p2 <-
  ...
  geom_area(data = BinomialGrid %>% filter(p < .75 & p > .5)) +
  labs(x = "proportion of water (p)",
       y = "density")
...
```{r}
(q_80 <- quantile(Samples6of9$p, prob = .8))
# equivalent, using mosaic::quantile()
quantile(~ p, data = Samples6of9, prob = .8)
```

That value will come in handy for the lower left panel of Figure 3.2. For an alternative approach, we could
...
```{r}
Samples6of9 %>% 
  summarise(`80th percentile` = quantile(p, p = .8))
```
...
Samples6of9 %>% 
  summarise(`10th percentile` = quantile(p, p = .1),
            `90th percentile` = quantile(p, p = .9))
```
...
```{r}
(q_10_and_90 <- quantile(Samples6of9$p, prob = c(.1, .9)))
```
...
Now we follow along with McElreath's **R** code 3.11 to compute a highly skewed posterior. We've already defined `p` and `prior` within `d`, above. Here we'll reuse them and update the rest of the columns.
...
The `rethinking::PI()` function works like a nice shorthand for `quantile()`.  So does `mosaic::cdata()`.

```{r}
quantile(Samples3of3$p, prob = c(.25, .75))

rethinking::PI(Samples3of3$p, prob = .5)

cdata( ~ p, data = Samples3of3, p = 0.5)
```

Now's a good time to introduce Matthew Kay's [-@R-tidybayes] [**tidybayes** package](https://mjskay.github.io/tidybayes/), which offers an array of convenience functions for summarizing Bayesian models of the type we'll be working with in this project. For all the **brms**-related details, see Kay's [-@kayExtractingVisualizingTidy2020] vignette, [*Extracting and visualizing tidy draws from brms models*](https://mjskay.github.io/tidybayes/articles/tidy-brms.html). Here we start simple.

```{r, message = F, warning = F}
library(tidybayes)

median_qi(Samples3of3$p, .width = .5)
```
...
```{r}
median_qi(Samples3of3$p, .width = c(.5, .8, .99))
```
...
```{r}
rethinking::HPDI(Samples3of3$p, prob = .5)
```
...
```{r}
mode_hdi(Samples3of3$p, .width = .5)
```
...
```{r}
qi(Samples3of3$p, .width = .5)
hdi(Samples3of3$p, .width = .5)
```

These are nice in that they return simple numeric vectors, making them particularly useful to use as references within 
plots. Now we have that skill, we can use it to make Figure 3.3.
...
<!-- First we'll update the simulation for six waters in nine tosses.  -->
...
```{r}
BinomialGrid3of3 %>% 
  summarise(`expected loss` = sum(posterior * abs(0.5 - p)))
```
...
```{r}
Samples3of3 %>% 
  summarise(posterior_median = median(p))
```
...
Based on quadratic loss $(d - p)^2$, the exact minimum value is `r min_loss[1]`.
Within sampling error, this is the posterior mean of our samples.

```{r}
Samples3of3 %>% 
  summarise(posterior_meaan = mean(p))
```
...
would be the disaggregated version of just one of the numerals returned by `rbinom()` when `size = 9`. So let's try simulating again with un-aggregated samples. We'll keep adding to our `samples` tibble. In addition to the disaggregated `draws` based on the $p$ values listed in `p`, we'll also want to add a row index for each of those `p` values--it'll come in handy when we plot.
